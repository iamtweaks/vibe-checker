...
import { redactFindings, redactTargetUrl } from './redaction'
...
export function generatePDF(result: ScanResult): jsPDF {
  ...
  doc.text('VibeCode Scanner Security Report', margin, 25)
  ...
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(10)
  doc.text(redactTargetUrl(result.targetUrl), margin, y)
  y += 10
  ...
  const sortedFindings = [...redactFindings(result.findings)].sort((a, b) => {
  ...
  doc.setFontSize(8)
  doc.text('Generated by VibeCode Scanner - Free Vibe Coding Security Scanner', pageWidth / 2, footerY, { align: 'center' })
...
export function downloadPDF(result: ScanResult) {
  ...
  const filename = `vibecode-scanner-report-${result.type}-${new Date(result.scannedAt).toISOString().slice(0, 10)}.pdf`
  doc.save(filename)
}
